...
description: Diagnoses and fixes slow Neo4j Cypher queries by reading execution plans, identifying
  ...
  Does NOT write new Cypher from scratch — use neo4j-cypher.
  Does NOT cover GDS algorithm tuning — use neo4j-gds-skill.
  Does NOT cover index/constraint creation syntax details — use neo4j-cypher references/indexes.md.
...
Run EXPLAIN/PROFILE and diagnostic Cypher through the Neuronet MCP tools — `mcp__neo4j-neuronet__read_neo4j_cypher` / `write_neo4j_cypher` / `get_neo4j_schema` — not a plain `neo4j` MCP server.
...
## When NOT to Use
- **Writing Cypher from scratch** → `neo4j-cypher`
