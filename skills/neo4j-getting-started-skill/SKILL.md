...
This environment connects Neo4j through the **Neuronet** MCP server. Wherever a stage below references `mcp__neo4j__read-cypher` / `write-cypher` / `get-schema`, use `mcp__neo4j-neuronet__read_neo4j_cypher` / `write_neo4j_cypher` / `get_neo4j_schema` instead.
...
- **Cypher authoring on existing project** → `neo4j-cypher`
