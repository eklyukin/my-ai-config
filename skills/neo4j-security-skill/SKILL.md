...
description: Programmatic security management in Neo4j — RBAC/ABAC, user lifecycle (CREATE/ALTER/DROP USER),
  ...
  — use neo4j-cypher. Does NOT handle cluster ops or backups — use neo4j-cli-tools-skill.
...
Run the RBAC/ABAC Cypher below through the Neuronet MCP tools already connected here (`mcp__neo4j-neuronet__read_neo4j_cypher`, `write_neo4j_cypher`, `get_neo4j_schema`), not a plain `neo4j` MCP server.
...
## When NOT to Use
- **Writing Cypher queries against application data** → `neo4j-cypher`
