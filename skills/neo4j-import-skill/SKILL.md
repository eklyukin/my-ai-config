...
description: Import structured data into Neo4j — LOAD CSV, CALL IN TRANSACTIONS, neo4j-admin
  ...
  (MERGE/CREATE) — use neo4j-cypher. Does NOT handle neo4j-admin backup/restore/config
...
For validating an import (row counts, spot-checking loaded nodes/rels) via Cypher, use the Neuronet MCP tools already connected here (`mcp__neo4j-neuronet__read_neo4j_cypher`, `write_neo4j_cypher`, `get_neo4j_schema`).
...
- **Live application writes (MERGE/CREATE in app code)** → `neo4j-cypher`
...
1. **Create uniqueness constraints** (enables index used by MERGE):
   ...
   ```
   > **Neo4j 2026.02+ (Enterprise/Aura) — PREVIEW:** `ALTER CURRENT GRAPH TYPE SET { … }` can replace all individual constraint statements with a single declarative block. See `neo4j-cypher/references/graph-type.md`. Use individual `CREATE CONSTRAINT` on older versions or Community Edition.
...
- [Indexes and constraints — types, MERGE lock semantics, import pre-flight](../neo4j-cypher/references/indexes.md)
