...
description: Use when working with Neo4j command-line tools — neo4j-cli (modern unified
  ...
  Does NOT cover Cypher query authoring — use neo4j-cypher.
...
For Cypher execution and schema inspection specifically, prefer the Neuronet MCP tools already connected here (`mcp__neo4j-neuronet__read_neo4j_cypher`, `write_neo4j_cypher`, `get_neo4j_schema`) over any of the CLIs below — no separate auth needed. The CLIs below remain relevant for backup/restore/import/CI use cases they cover that Neuronet doesn't.
...
- **Writing or optimizing Cypher queries** → `neo4j-cypher`
...
neo4j-cli skill install neo4j-cypher       # one catalog skill, all agents
