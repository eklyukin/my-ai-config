...
description: Use when installing, configuring, or troubleshooting the official Neo4j MCP server
  ...
  Does NOT cover writing Cypher queries via those tools — use neo4j-cypher.
...
This environment already connects to Neo4j through the **Neuronet** MCP server (`mcp__neo4j-neuronet__get_neo4j_schema`, `read_neo4j_cypher`, `write_neo4j_cypher`) rather than the plain `neo4j/mcp` server this skill installs by default. Use the Neuronet tools directly for query execution; treat the install/config instructions below as relevant only when setting up a *new*, non-Neuronet MCP connection (e.g. for another editor). `neuronet_code_search` is a separate Neuronet capability for indexed source code, not this graph — see `rules/neo4j-graph-first.md`.
...
- **Writing or optimizing Cypher queries** → use `neo4j-cypher`
