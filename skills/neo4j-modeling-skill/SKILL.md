...
description: Design, review, and refactor Neo4j graph data models. Use when choosing node
  ...
  Does NOT handle Cypher query authoring — use neo4j-cypher.
...
When validating a model against the live graph (checking constraints/indexes, inspecting actual data), use the Neuronet MCP tools already connected here (`mcp__neo4j-neuronet__read_neo4j_cypher`, `write_neo4j_cypher`, `get_neo4j_schema`), not a plain `neo4j` MCP server.
...
- **Writing or optimizing Cypher** → `neo4j-cypher`
...
11. **On Neo4j 2026.02+ (Enterprise/Aura):** consider `ALTER CURRENT GRAPH TYPE SET { … }` or `EXTEND GRAPH TYPE WITH { … }` to declare the full model in one block instead of individual `CREATE CONSTRAINT` statements — see `neo4j-cypher/references/graph-type.md`. **PREVIEW** — syntax may change before GA.
